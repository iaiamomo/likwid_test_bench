...
#define DEFAULT_NB_SUPPORTER_THREADS	0
...
long set_contains(TM_ARGDECL long  val)
{
  ...
  unsigned int numSupportedThreads;
  unsigned int seed;
  long operations;
...
MAIN(argc, argv) {
  ...
  struct option long_options[] = {
    ...
    {"num-supp-threads",          required_argument, NULL, 'z'},
    {NULL, 0, NULL, 0}
  };
  ...
  numSupportedThreads = DEFAULT_NB_SUPPORTER_THREADS;
  range = DEFAULT_RANGE;
    ...
    switch(c) {
     ...
     case 'h':
       printf("intset -- STM stress test "
              ...
              "  -z, --num-supp-threads <int>\n"
              "        Number of threads (default=" XSTR(DEFAULT_NB_SUPPORTER_THREADS) ")\n"
         );
       exit(0);
     case 'd':
     ...
     case 'u':
       ...
       break;
     case 'z':
    	 numSupportedThreads = atoi(optarg);
       break;     case '?':
  ...
  TM_STARTUP(nb_threads, numSupportedThreads);
